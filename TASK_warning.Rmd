...
if [ `cat PROJECTDIR/subjects/SUBID/sessionNO/0_group` == "CONTROL" ]; then
   ...
   ME_RS_on=`grep ME_RS_10min_On PROJECTDIR/subjects/SUBID/sessionNO/QA/SUBID_fMRIpar.csv | awk -F',' '{print $NF}'`
   ME_Task_on=`grep ME_Task_10min_On PROJECTDIR/subjects/SUBID/sessionNO/QA/SUBID_fMRIpar.csv | awk -F',' '{print $NF}'`
else
   rest-on_regDICE=`grep 'value="' PROJECTDIR/subjects/SUBID/sessionNO/rest-on/rest-on_regDICE | awk '{print $7}' | grep -o '[0-9].[0-9]*'`
   axcpt-on_regDICE=`grep 'value="' PROJECTDIR/subjects/SUBID/sessionNO/axcpt-on/axcpt-on_regDICE | awk '{print $7}' | grep -o '[0-9].[0-9]*'`
   ...
   ME_RS_on=`grep ME_RS_10min_On PROJECTDIR/subjects/SUBID/sessionNO/QA/SUBID_fMRIpar.csv | awk -F',' '{print $NF}'`
   ME_Task_on=`grep ME_Task_10min_On PROJECTDIR/subjects/SUBID/sessionNO/QA/SUBID_fMRIpar.csv | awk -F',' '{print $NF}'`
   ME_RS_off=`grep ME_RS_10min_Off PROJECTDIR/subjects/SUBID/sessionNO/QA/SUBID_fMRIpar.csv | awk -F',' '{print $NF}'`
   ME_Task_off=`grep ME_Task_10min_Off PROJECTDIR/subjects/SUBID/sessionNO/QA/SUBID_fMRIpar.csv | awk -F',' '{print $NF}'`
fi
```
...
ME_RS_on.warn <- if (ME_RS_on == 1) {print("check ME_RS_On scan parameters")}
ME_RS_off.warn <- if (ME_RS_off == 1) {print("check ME_RS_Off scan parameters")}
ME_Task_on.warn <- if (ME_Task_on == 1) {print("check ME_Task_On scan parameters")}
ME_Task_off.warn <- if (ME_Task_off == 1) {print("check ME_Task_Off scan parameters")}
...
<span style="color:red">`r ME_RS_on.warn`</span><br>
<span style="color:red">`r ME_RS_off.warn`</span><br>
<span style="color:red">`r ME_Task_on.warn`</span><br>
<span style="color:red">`r ME_Task_off.warn`</span><br>
